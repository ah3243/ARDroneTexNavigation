...
int filterHandle(Mat &in, Mat &out, vector<vector<Mat> > filterbank, int n_sigmas, int n_orientations, int counter){
  ...
  if(SAVEAGGRESPONSES){
    ...
    imshow("out", out);
    stringstream ss;
    ss << "./FilterResponses/in" << counter << ".jpg";
    imwrite(ss.str(), in);
    ss.str(string());
    cout << "This is the str: " << ss.str() << endl;
    ss << "./FilterResponses/out" << counter << ".jpg";
    imwrite(ss.str(), out);
  }
  return 1;
}
