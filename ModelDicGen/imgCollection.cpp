...
void getRemapMats(Mat &map1, Mat &map2){
  Mat cameraMatrix, distCoeffs;
  double image_Width, image_Height;

  // Get Distortion Coefficients and Camera Matrix from file
  cout << "Remapping Images" << endl;
  FileStorage fs("/home/james-tt/Desktop/MyFilterbankCode/imageRemapping/calFile.xml", FileStorage::READ);
...
// Import adjust images to known distortion and rescale, save modified images to output map
void loadClassImgs(path p, map<string, vector<Mat> > &classImgs, int scale){
    ...
    // Calculate remaping matricies based on previous lens calibration and save to Mat
    Mat remapMat1, remapMat2;
    getRemapMats(remapMat1, remapMat2);
    ...
    for(directory_iterator itr(p); itr != itr_end; ++itr){
      ...
      // Remap image to adjust for lens distortion
      Mat imgRec = in;
      remap(in, imgRec, remapMat1, remapMat2, INTER_LINEAR);

      // Downscale image to required size
      Mat imgsm;
      scaleImg(imgRec, imgsm, scale);

      // Extract image class name and store in map
      extractClsNme(nme);
